...
module.exports.run = async (client, message, args) => {
	...
	if (!checkTarget) {
		return message.channel.send(
			new Discord.MessageEmbed().setTitle(':warning: Error').setDescription('That user doesn\'t even exist. Not even sure what you\'re on about').setColor('RED'),
		);
	}

	const nuggiesStaff = await staff.findOne({ _id: GuildID });
	...
	if (nuggiesStaff) {
		if (nuggiesStaff.Developers.includes(target.id)) {
			...
			nuggiesStaff.Developers.addToSet(`${target.id}`);
		...
		const newE = new staff({ _id: GuildID, Developers: [`${target.id}`] });

		await newE.save().catch((error) => console.log(error));

		message.channel.send(new Discord.MessageEmbed().setTitle('<a:9689_tick:785181267758809120> Success!').setDescription(`**<@${target.id}>** was successfully given developer permissions`).setColor('GREEN'));
		message.guild.members.cache.get(target.id).send(
			new Discord.MessageEmbed().setTitle(':warning: Alert').setDescription(`You were granted developer permissions by **${message.author.tag}** for **Nuggies**`).setColor('GREEN'));
	}
};
