...
module.exports.run = async (client, message, args, utils, data) => {
	...
	osutils.cpuUsage(function(v) {
		const embed = new Discord.MessageEmbed()
			...
			.addField('Global Prefix <:VerifiedBotDeveloper:801694881726332998>', `\`\`\`${globalprefix}\`\`\``, true)
			.addField('Total Servers <a:9689_tick:785181267758809120>', `\`\`\`${client.guilds.cache.size}\`\`\``, true)
			.addField('Support Server <:SupportThumb:811209722883473430>', "[Click To Join](https://discord.gg/BxZUrr3a3u)", true)
			.addField('Library <:nodeJS:785064545013858334>', '```Discord.js v12```', true)
			...
			.addField('RAM Usage Of VPS', `\`\`\`${(osutils.totalmem() - osutils.freemem()).toString().split('.')[0]}/${osutils.totalmem().toString().split('.')[0]} MB\`\`\``, true)
			.addField('RAM Usage Of Bot', `\`\`\`${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}` + '/' + `${osutils.totalmem().toString().split('.')[0]}` + ' MB\`\`\`', true)
